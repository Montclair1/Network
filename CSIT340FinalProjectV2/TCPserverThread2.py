...
# PROGRAM WORKS 4/26 with multiple threads, but server does not exiting properly, threads keep running
...
def threaded(connectionSocket):
    while True:  # data received from TCPclient
        ...
        if message:  # if message exists, calculate expression
            ...
            sys.exit() # close thread TEST  ????  NOT CLOSING
        #else:
        #   print_lock.release()
        #   print("print lock release")
        #   break
...
while True:
	...
	#close the connection where there are no more requests
        ...
        #exit(0)  # works 3/6/18
        exit()
        #sys.exit()   #EXITS IN start_new_thread method delete finally?
